...
public class MeshDestroy : MonoBehaviour
{
    ...
    void Update()
    {
        if (isTest && Input.GetMouseButtonDown(2))
        {
            DestroyMesh();
        }
    }
    ...
    public void DestroyMesh()
    {
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            ...
            parts[i].GameObject.GetComponent<Rigidbody>().AddForceAtPosition(parts[i].Bounds.center * ExplodeForce + (Vector3.up) * 500, transform.position);
            Destroy(parts[i].GameObject, 2);
        }
    ...
    public class PartMesh
    {
        ...
        public void MakeGameobject(MeshDestroy original)
        {
            ...
            newMat[0] = original.GetComponent<MeshRenderer>().materials[0]; 
